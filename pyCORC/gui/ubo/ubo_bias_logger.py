...
class ubo_logger(QObject):
    ...
    def __init__(self,init_args,session_data):
        ...
        self.save_path = os.path.join(os.path.dirname(__file__), '../../..',f"logs/pycorc_recordings/{self.subject_id}")
    ...
    def reset_logger(self):
        ...
        if self.init_args["corc"]["on"]:
            if hasattr(self, 'corc_response'):
                ...
                for i in range(3):
                    if i == 0:
                        bias = np.mean(total_arr[:,1+i*6:1+i*6+6],axis=0)+np.array([0,-0.82,0,0,0,0])
                    else:
                        bias = np.mean(total_arr[:,1+i*6:1+i*6+6],axis=0)+np.array([0,0,-0.82,0,0,0])
                ...
                self.save_file(path=f"{self.save_path}/",df=df,item=f"UBOBias{self.take_num+1}Log")
        # emit the signal
        self.time_ready.emit({
