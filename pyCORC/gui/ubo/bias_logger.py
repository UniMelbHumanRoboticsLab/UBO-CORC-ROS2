...
class bias_logger(QObject):
    ...
    def __init__(self,init_args,session_data):
        ...
        self.body_path = os.path.join(os.path.dirname(__file__), '../../..',f'logs/pycorc_recordings/{self.exp_id}/subject_measurements/{self.subject_id}')
        self.body_params_rbt,self.ft_grav,self.removed_bias = get_subject_params(self.body_path)
        self.skeleton = ub(self.body_params_rbt,model="ubo",arm_side="right")
        self.sensors_ready_flag = False
        self.collect_bias = True
    ...
    def log_current_data(self):
        ...
        """
        if hasattr(self, 'corc_response') and hasattr(self, 'xsens_response') and self.collect_bias:
            ...
            for i in range(NUM_RFT):
                ...
                force_data = np.array(corc_data[1+i*6:1+i*6+6])+cur_removed_bias
    ...
    def reset_logger(self):
        if hasattr(self, 'corc_response') and hasattr(self, 'xsens_response') and self.collect_bias:
            print(f"\nBias Take Elapsed Time: {self.elapsed_time}")
            ...
            self.save_file(path=f"{self.body_path}/",df=df,item="UBORecordBiasLog")
            ...
            self.save_file(path=f"{self.body_path}/",df=df,item="UBOConstBiasLog")
            np.save(f"{self.body_path}/UBOAvgBias", mean_bias)
            
            print(f"Finished collecting {self.subject_id} Bias")
            self.collect_bias = False
